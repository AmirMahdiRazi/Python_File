...
cnx = mysql.connector.connect(user="root",password="Razi0023161469",host="127.0.0.1",database="UP")
...
while b2:
    ...
    for i in range(len(Password)):
        if (Password[i] in alfa_low or Password[i] in alfa_up or Password[i] in numb):
           count+=1 
        else:
            print("The password must contain numbers and letters")
    if count==len(Password):
        b2=False


crusor.execute('insert into UP values (\'%s\',\'%s\')' % (Username,Password))
cnx.commit()    
